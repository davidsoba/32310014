...
knot_opt
```
Para determinar el número óptimo de knots para el problema de regresión spline mediante validación cruzada en 10 folds, realizamos ajustes de modelos para cada valor de knots (de 1 a 10) y evaluamos su rendimiento en términos de error cuadrático medio (ECM) en el conjunto de prueba. Luego, calculamos el promedio de los ECM por cada fold. 

El fold óptimo es el número 8, y el knot óptimo que minimiza el ECM es también 8. Esto implica que el modelo con 8 knots resulta en el menor ECM de predicción según la validación cruzada en 10 folds. Lo que significa que utilizar 8 knots en el modelo de regresión spline proporciona el mejor ajuste a los datos de entrenamiento. 
...
Después de realizar la validación curazada en 10 folds y calcular los ECM para cada uno de los tres modelos, los resultados son los siguientes:
...
Basándonos en el ECM de prueba, el mejor modelo sería *el mejor modelo basado en base de funciones*, es decir, la regresión spline óptima. Este modelo proporciona el menor error de predicción en el conjunto de prueba.
