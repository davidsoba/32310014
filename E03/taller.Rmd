---
title: "Taller 3 - Modelos generalizados. Regresión logística, variables latentes y datos estructurados."
...
$$P(x|\theta)= \begin{cases} \theta & \text{si } x = 1 \\ 1-\theta & \text{si } x = 0 \end{cases}$$
...
*DISTRIBUCIÓN NORMAL:*
$$p(x|\sigma) = \frac{1}{\sqrt{2\pi\sigma^{2}}} \ e^{\left(-\frac{1}{2\sigma^{2}} \cdot (x-\mu)^{2} \right)}$$

$$p(x|\sigma) = \frac{1}{\sqrt{2\pi\sigma^{2}}} \ exp^{\left(-\frac{x^{2}}{2\sigma^2}+\frac{x\mu}{\sigma^{2}}-\frac{\mu^{2}}{2\sigma^{2}}\right)}$$

$$p(x|\sigma) = \left[ \frac{1}{\sqrt{2\pi\sigma^{2}}} \exp{\left(-\frac{x^{2}}{2\sigma^2}\right)} \right] \cdot \exp \left( \frac{\mu}{\sigma^{2}}x-\frac{\mu^{2}}{2\sigma^{2}} \right)$$

$$p(x|\sigma) = \frac{1}{\sqrt{2\pi}} \cdot \exp \left( \frac{\mu}{\sigma^{2}}x-\frac{x^{2}}{2\sigma^{2}}-\frac{\mu^{2}}{2\sigma^{2}}- \frac{1}{2} log (2\pi\sigma^{2}) \right)$$
...
$$P(x|\lambda) = \frac{1}{x!}\cdot (\lambda^x\cdot e^{-\lambda})$$

$$P(x|\lambda) = \frac{1}{x!}\cdot (exp(log(\lambda^{x}))\cdot e^{-\lambda})$$
...
Se observa una diferencia significativa en la proporción de enfermedad cardíaca entre hombres y mujeres, donde la proporción de casos positivos es más alta en hombres que en mujeres. En cuanto al tipo de dolor en el pecho reportado, se evidencia una variabilidad en la proporción de enfermedad cardíaca, siendo los pacientes con dolor de pecho atípico (valor 2) o asintomáticos (valor 4) los que tienen una mayor proporción de casos positivos para enfermedad cardíaca. Por otro lado, no parece haber una diferencia significativa en la proporción de enfermedad cardíaca entre pacientes con azúcar en sangre en ayunas mayor o igual a 120 mg/dl (valor 1) y aquellos con azúcar en sangre en ayunas menor a 120 mg/dl (valor 0). Además, la proporción de enfermedad cardíaca parece variar según los resultados electrocardiográficos en reposo, la presencia de angina inducida por el ejercicio y la pendiente del segmento ST durante el ejercicio.Respecto al resultado del test de talio, se observa una variabilidad en la proporción de enfermedad cardíaca, siendo los pacientes con un resultado de talio 7 los que parecen tener una mayor proporción de casos positivos para enfermedad cardíaca.
...
```
En el modelo multivariado ajustado, se observa que las variables ´sex´, ´cp´,´trestbps´, ´exang´, ´ca´ y ´thal´ son significativas según el test de Wald,  con valores p < 0.05, lo que indica que tienen un impacto significativo en la presencia de enfermedad cardíaca. Por otro lado, las variables ´age´, ´chol´, ´fbs´,´restecg´, ´thalach´, ´oldpeak´ y ´slope´ no son significativas con valores p >= 0.05, lo que sugiere que no contribuyen de manera significativa a la predicción de la enfermedad cardíaca. 
